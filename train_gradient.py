...
from poison_loader import CIFAR10dirty, folder_load, CIFAR10grad
...
def main(args):
    ...
    data_set = CIFAR10grad('../data/', args.pr, transform=transform_train)
    ...
    ids = []
    with open('results/ResNet18-grad-Linf-eps16.00000/target_ids.txt', 'r') as f:
        for line in f:
            ids.append(int(line))
    target_set = torch.utils.data.Subset(test_set, ids)
    ...
    target_loader = DataLoader(target_set, batch_size=args.batch_size, shuffle=False, num_workers=8)
    ...
    if not os.path.isfile(args.model_save_path):
        ...
        train_model(args, model, optimizer, schedule, train_loader, val_loader, target_loader, writer)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--poison_type', default='C', choices=['C', 'P1', 'P2', 'P3', 'P4', 'P5', 'TAP', 'grad'])
