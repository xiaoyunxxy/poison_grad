...
def batch_poison_feature(model, x, target, args, adv_or_hyp, dirty_model):
    orig_x = x.clone().detach()
    step = STEPS[args.constraint](orig_x, args.eps, args.step_size)
    ...
    for _ in range(args.num_steps):
        x = x.clone().detach().requires_grad_(True)
        logits, feature = model(x, with_latent=True)
        logits_dirty, feature_dirty = dirty_model(x, with_latent=True)

        fea_norm = torch.norm((feature_dirty-feature), dim=1, p=2).sum()

        # print(fea_norm)
        # print('------', fea_norm)

        loss = nn.CrossEntropyLoss()(logits, target) + 0.5*fea_norm
...
def poison_p6_adv(args, loader, model, writer, adv_or_hyp, dirty_model):
    poisoned_input = []
    clean_target = []
    loss_logger = AverageMeter()
    acc_logger = AverageMeter()
    ...
    for i, (inp, target) in iterator:
        ...
        inp_p = batch_poison_feature(model, inp, target, args, adv_or_hyp, dirty_model)
...
def poison_p6(args, loader, model, dirty_model, writer):
    poisoned_data = poison_p6_adv(args, loader, model, writer, adv_or_hyp='adv', dirty_model=dirty_model)
    return poisoned_data
...
if __name__ == "__main__":
    ...
    parser.add_argument('--poison_type', default='C', choices=['P1', 'P2', 'P3', 'P4', 'P5', 'P6'])
